...
function SortableScene({ id, children }: { id: string; children: React.ReactNode }) {
...
export default function App() {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (!over || active.id === over.id || !activeProject || !activeProjectId) return;

    const oldIndex = parseInt((active.id as string).split('-')[1]);
    const newIndex = parseInt((over.id as string).split('-')[1]);

    const newScenes = arrayMove(activeProject.scenes || [], oldIndex, newIndex);
    
    try {
      await updateDoc(doc(db, 'projects', activeProjectId), {
        scenes: newScenes
      });
      showNotification("場景順序已更新！");
    } catch (error) {
      console.error(error);
      showNotification("更新順序失敗。");
    }
  };
                  ...
                  {!activeProject?.scenes || activeProject.scenes.length === 0 ? (
                    ...
                    <div className="space-y-6">
                      <DndContext collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                        <SortableContext items={activeProject.scenes.map((_, idx) => `scene-${idx}`)} strategy={verticalListSortingStrategy}>
                          {activeProject.scenes.map((scene: any, idx: number) => {
                            ...
                            return (
                              <SortableScene key={sceneId} id={sceneId}>
                                <div className="bg-black/60 border border-neon-cyan/20 rounded-2xl p-5 hover:border-neon-cyan/50 transition-all shadow-[0_4px_15px_rgba(0,0,0,0.3)]">
                                  {/* Scene header */}
                                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 pb-3 border-b border-gray-800 mb-4">
                                    <div className="flex items-center gap-2.5">
                                      <span className="bg-neon-cyan text-black text-xs font-bold px-2 py-0.5 rounded">
                                        ...
                                        <GripVertical className="inline w-3 h-3 ml-1 cursor-grab" />
                                      </span>
                                      ...
                                      {scene.imageUrl ? (
                                        <div className="relative w-full h-full group overflow-hidden">
                                          ...
                                          <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/95 via-black/60 to-transparent p-2.5 flex items-center justify-between gap-2 z-10">
                                            <div className="text-[10px] text-gray-300 font-mono tracking-wider flex items-center gap-1.5">
                                              {scene.aiVideoUrl ? (
                                                ...
                                                {videoTasks[idx] ? (
                                                  <>
                                                    {videoTasks[idx].status === 'failed' ? (
                                                      <span className="text-red-500">生成失敗: {videoTasks[idx].error || "未知錯誤"}</span>
                                                    ) : (
                                                      <>
                                  ...
                                  <div className="mt-4 pt-4 border-t border-gray-900 space-y-3">
                                    ...
                                    <div className="bg-gray-950/40 rounded-xl p-3.5 border border-gray-800/80 space-y-3">
                                      ...
                                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-1.5">
                                        <a href="https://seedance2.ai" target="_blank" rel="noreferrer referrer" className="py-2 bg-gray-900 hover:bg-neon-cyan/10 border border-neon-cyan/40 hover:border-neon-cyan text-neon-cyan text-[10px] font-bold rounded-lg text-center transition-all flex items-center justify-center gap-1 shadow-xs" title="Seedance 2 AI 影片生成（熱門推薦）">🚀 Seedance 2</a>
                                        <a href="https://aitestkitchen.withgoogle.com/tools/video-fx" target="_blank" rel="noreferrer referrer" className="py-2 bg-gray-900 hover:bg-neon-pink/10 border border-neon-pink/40 hover:border-neon-pink text-neon-pink text-[10px] font-bold rounded-lg text-center transition-all flex items-center justify-center gap-1 shadow-xs" title="Google VideoFX (Veo 3) 免費創作空間">🔮 Google Veo 3</a>
                                        <a href="https://lumalabs.ai/dream-machine" target="_blank" rel="noreferrer referrer" className="py-2 bg-gray-900 hover:bg-yellow-500/10 border border-yellow-500/40 hover:border-yellow-500 text-yellow-500 text-[10px] font-bold rounded-lg text-center transition-all flex items-center justify-center gap-1 shadow-xs" title="Luma Dream Machine 免費渲染器">⚡ Luma Labs</a>
                                        <a href="https://klingai.com" target="_blank" rel="noreferrer referrer" className="py-2 bg-gray-900 hover:bg-green-500/10 border border-green-500/40 hover:border-green-500 text-green-500 text-[10px] font-bold rounded-lg text-center transition-all flex items-center justify-center gap-1 shadow-xs" title="Kling AI 快手可靈免費通道">🌟 Kling AI</a>
                                      </div>
                                    </div>
                                  </div>
                                </div>
